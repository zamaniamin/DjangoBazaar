...
# TODO move saving options logic to the new service class and use it on creating products and products options
class ProductOptionService:
    ...
    @classmethod
    def create_option(cls, product, option_name: str, new_items: list):
        ...
        product_option, created = ProductOption.objects.get_or_create(
            product=product, option_name=option_name
        )
        current_items = set(product_option.items.values_list("item_name", flat=True))
        ...
        for item_name in items_to_add:
            ProductOptionItem.objects.create(option=product_option, item_name=item_name)

        # Remove items no longer present
        product_option.items.filter(item_name__in=items_to_remove).delete()
